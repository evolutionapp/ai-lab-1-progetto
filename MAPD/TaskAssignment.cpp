...
void TaskAssignment::initializeAssignments(){
    ...
    for(Agent* a : agents->agents ){
        // Insieme A = [a1,a2,a3,...]
        //ak = {ok,path,TTD}
        //ok=sequenza ordinata di azioni

        //Inizializzo ogni robot con l'azione iniziale e finale 
        ...
        //Inizializza l'agente e il costo ideale
...
bool TaskAssignment::assignTasks() {
    ...
    int assegnamenti[NUM_AGENTS][NUNM_TASKS];
    ...
    while (!unassigned_tasks.empty()){
        ...
        assegnamenti[agent_id][task_id]=1;
        ...
        //Aggiorna il path dell'assegnamento nel caso in cui non sia vuoto
        ...
        //Rimuovo dai task non assegnati quello appena assegnato
        unassigned_tasks.erase(min_cost_assign->new_add_task->heap_handle);

        //Aggiungo ai task assegnati quello appena assegnato
        assignedTasks.insert(min_cost_assign->new_add_task);

        //Elimina il task dall'heap
        delete_top(task_id);

        //Aggiorna tutti gli Heap 
...
int TaskAssignment::haveConflict(int agent_id, vector<PathEntry>& path){
    for (int i = 1 + (start_timestep>=0?start_timestep:0); i<path.size();i++){
        //se il percorso è "vincolato" allora ho un conflitto al tempo i rispetto ad timestamp corrente
...
Assignment* TaskAssignment::insertTask(Agent* a,Task* task, bool real_cost){
    /*Per l'agente passato in input prova a inserire le azioni di pick up e ddrop off del task passato in input. Le prova a mettere in una qualsiasi posizione all'interno
    della sequenza di azioni del robot
    aggiorna poi le azioni del robot*/
...
//            }
            ...
            //Controllo la differenza di tempo prima e dopo l'inserimento dell'azione nel piano di azioni del robot           
...
void TaskAssignment::printAssignments(){
    for (auto& a: assignments){
        cout<<"Agent: "<< a.agent->agent_id<<", Cost: "<<a.ideal_cost<<", Capacity: "<<a.agent->capacity<< ", Task amount: "<<a.assigned_tasks.size()<<", Actions:"<<endl;
        a.print();
    }
}
