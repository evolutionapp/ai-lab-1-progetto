...
public:
    ...
    virtual Assignment *get_assignment(int agentId,int taskId) {};
    ...
    void printPath(){
        for(int agent_id = 0;agent_id<agents->agents.size();agent_id++){
            cout<<"Agent: "<<agent_id<<", Delay: "<< assignments[agent_id].current_total_delay<<" , Cost: "<<assignments[agent_id].path.size()-1 << ", plan: "<<endl;
            for (auto& entry : assignments[agent_id].path){
                // cout<<"ENTRY LOCATION : "<<entry.location<<endl;
                // cout<<"MAP COLS : "<<map->cols<<endl;
                cout<<"\t\t"<<entry.timeStep<<" : "<<"("<<entry.location/map->cols<<","<<entry.location%map->cols<<")->"<<endl;
            }
            cout<<endl;
        }
    }
